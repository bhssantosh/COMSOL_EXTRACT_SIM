...
class COMSOLMesh:
    ...
    def _parse(self, filepath: str):
        ...
        while i < n_lines:
            ...
            try:
                ...
                if re.match(r"\d+\s*#\s*number of element types", line, re.I):
                    break  # stay at this line for element parsing
                else:
                    i += 1
                    break
        ...
        print(f"[COMSOLMesh] Loaded {len(self.nodes)} nodes (space dim = {self.spacedim})")
        ...
        while i < n_lines:
            ...
            if m:
                ...
                for _ in range(n_types):
                    # skip to "# Type #n"
                    while i < n_lines and not lines[i].strip().startswith("# Type"):
                        i += 1
                    if i >= n_lines:
                        break
                    i += 1  # to type name line
                    line = lines[i].strip()
                    parts = line.split()
                    if len(parts) >= 2:
                        type_num = int(parts[0])
                        elem_type = ['vtx', 'edg', 'tri', 'tet', 'hex', 'prism', 'pyr'][type_num]
                    ...
                    line = lines[i].strip()
                    parts = line.split()
                    n_elems = int(parts[0]) if parts else 0
                    i += 1

                    # skip to "# Elements"
                    while i < n_lines and not lines[i].strip().startswith("# Elements"):
                        i += 1
                    i += 1
                    ...
                    connectivity = np.array(conn_rows, dtype=int) if conn_rows else np.empty((0, n_npe), dtype=int)

                    # Skip to domain line (Accommodates different COMSOL versions)
                    while i < n_lines and not re.search(r"\d+\s*#\s*number of (geometric entity indices|domains)", lines[i], re.I):
                        i += 1
                    
                    if i >= n_lines:
                        domains = np.ones(n_elems, dtype=int)
                    else:
    ...
    def summary(self):
        ...
        for sec in self.sections:
            ...
            print(f"    [{sec.elem_type:6s}] {len(sec.connectivity):6d} elems  domains: {doms}")
        print("=" * 55)
    ...
    def get_boundary_points(
        ...
        if elem_types is None:
            ...
            for t in surface_types:
                if self.spacedim == 3 and t in ("tri", "tri2", "quad", "quad2", "edg", "edg2", "vtx"):
        ...
        if not node_indices:
            print(f"[Warning] No nodes found for tag={tag}, elem_types={elem_types}.")
            return np.empty((0, 3))
...
if __name__ == "__main__":
    ...
    # 1. Specify the mesh file path here (replace with your actual file path)
    mesh_file = "D:\\TAH\\Draft 1\\mesh_1.mphtxt"  

    # Wrap the execution block in a try-except to handle invalid paths gracefully
    try:
        # 2. Load mesh
        ...
        pts = mesh.get_boundary_points(tag=18)
        print(f"\nPoints on tag=18:\n{pts}")

        # 5. Extract by domain_index (e.g. second boundary in the sorted list)
        pts2 = mesh.get_boundary_points(domain_index=18)
        print(f"\nPoints on domain_index=18:\n{pts2}")

        # 6. Export to CSV
        mesh.export_boundary_points(tag=18, filename="side_18_points.csv")

        # 7. Plot  (comment out in headless environments)
        try:
            mesh.plot_boundary_points(tag=18, title="Side 18 – extracted points")
        ...
        print(f"\n[Error] Could not run demo. {e}")
        print("Please update 'mesh_file' variable with a valid path to your .mphtxt file.")